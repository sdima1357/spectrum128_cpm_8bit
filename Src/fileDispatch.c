	...
	else if(ev->message==MESS_REPAINT)
	{
		...
		else
		{
			...
			for(k = 0;k <NUmL;k++)
			{
				...
				int SYMB = LCD_getWidth()/CHAR_WIDTH-10;
				char buffer[SYMB+1];
				buffer[SYMB] = 0;
				memset(buffer,' ',SYMB);
				char * type[] = {"0:","T:","Z:","M:","N:"};
				LCD_Draw_Text("  ",xScr-16,yScr+(k)*CHAR_HEIGHT, WHITE, 1, BLACK);
				if(n<rfilesA)
				{
					...
					LCD_Draw_Text(type[strListArray[n][0]],xScr-16,yScr+(k)*CHAR_HEIGHT, (selection==n)?BLACK:WHITE, 1, (selection==n)?WHITE:BLACK);
				}
				LCD_Draw_Text(buffer,xScr,yScr+(k)*CHAR_HEIGHT, (selection==n)?BLACK:WHITE, 1, (selection==n)?WHITE:BLACK);
