...
#define TAPE_LINES     (SPECTRUM_HEIGHT+BORDER_HEIGHT*2)
#else
#define BORDER_WIDTH  32
#define BORDER_HEIGHT  4
#define TAPE_LINES (SPECTRUM_HEIGHT+SPECTRUM_HEIGHT/2+BORDER_HEIGHT*2)
#endif
...
void  poke(uint16_t addr,uint8_t value) 
{
	...
	switch(addr>>14u)
	{
		...
		case 3:
				...
				if(SINCLAIR_FLAGS.shadow_bank == SINCLAIR_FLAGS.ram_bank)
				{
...
int z48_z128_dialog_dispatch(struct SYS_EVENT* ev)
{
	...
	{
#ifdef ILI9341
#define LINE_SHIFT  (CHAR_HEIGHT+CHAR_HEIGHT/8)
#else
#define LINE_SHIFT  (CHAR_HEIGHT+CHAR_HEIGHT/2)
#endif
...
#ifdef ILI9341
uint16_t  block8[2][32*8*8];
#else
uint16_t  block12[2][32*12*12];
#endif

void paintLine(int line)
{
	if(line>23||!SINCLAIR_FLAGS.framePaint) return ;

#ifdef ILI9341
	int OffSX = (LCD_getWidth()  -SPECTRUM_WIDTH)/2;
	int OffSY = (LCD_getHeight() -SPECTRUM_HEIGHT)/2;
#else
	int OffSX = (LCD_getWidth()  -SPECTRUM_WIDTH*1.5)/2/1.5;
	int OffSY = (LCD_getHeight() -SPECTRUM_HEIGHT*1.5)/2/1.5;
#endif
		...
		if(!paint)
		{
			...
			for(xbb=0;xbb<32;xbb++)
			{
				...
				int tileAddr = xb+yb*32;
					...
					uint16_t* pp = &BKLC[attr|(!!(SINCLAIR_FLAGS.frameCounter&16))*256][0];
					...
					{
						colors[0] = __REV16(pp[0]);
						colors[1] = __REV16(pp[1]);
					}
					else
					{
						for(int k=0;k<2;k++)
						{
							uint16_t color0 = __REV16(pp[k]);
							uint8_t btt[3];
							color_convert(color0,btt);
							int FC = ((int)btt[0]+btt[1]+btt[2])/3;
							btt[0] = FC;btt[1] = FC;btt[2] = FC;
							colors[k] = color_convertRGB_to16(btt);
						}
					}
					{
						uint8_t btt0[3];
						color_convert(colors[0],btt0);
						uint8_t btt1[3];
						color_convert(colors[1],btt1);
						btt0[0] = (btt0[0]+btt1[0])/2;
						btt0[1] = (btt0[1]+btt1[1])/2;
						btt0[2] = (btt0[2]+btt1[2])/2;
						colors[2] = color_convertRGB_to16(btt0);
					}
#ifdef ILI9341
					makeBlock8(colors,buff,&block8[line&1][(xbb)*8],8*32);
#else
					makeBlock12(colors,buff,&block12[line&1][(31-xbb)*12],12*32);
#endif

			}
#ifdef ILI9341
			LCD_Write8x8line16_2_8_32(xScr,yScr,block8[line&1]);
#else
			LCD_Write8x8line16_2_12_32(xScr,yScr,block12[line&1]);
#endif
}
...
//				ev->message = MESS_CLOSE;
		...
		soundEvents(100);
		int clocksInLine = SINCLAIR_FLAGS.T69888/24;
		int line  = 0;
		uint16_t cnttt;
		SINCLAIR_FLAGS.frameDirty = 0;
		//paintLine(int line)
		{
			//z80_interrupt();
			for(int tt = clocksInLine;tt<SINCLAIR_FLAGS.T69888;tt+=clocksInLine)
			{
				z80_run(tt);
				if(line==2)
				{
		...
		if((cntt&0x1f)==1)
		{
			printf("tt=%d %d %d %d cnttt= %d\r\n",cputime,(curr_tick-prev_tick)/0x1f,sound_events,kkeyScanN,cnttt);
			prev_tick = curr_tick;
		}
		...
		if(SINCLAIR_FLAGS.frameDirty)
			SINCLAIR_FLAGS.framePaint = 1;
		else
			SINCLAIR_FLAGS.framePaint = 0;
		//SINCLAIR_FLAGS.frameDirty = 0;
		if((SINCLAIR_FLAGS.frameCounter&0xf)==1)
		{
			SINCLAIR_FLAGS.framePaint = 1;
		}
		ev->message=MESS_REPAINT;
	...
	else if(ev->message==MESS_REPAINT)
	{
		...
		//	waitScreenComplete();
			...
			int OffSY = (LCD_getHeight() -SPECTRUM_HEIGHT)/2;
#else
			int OffSX = (LCD_getWidth()  -SPECTRUM_WIDTH*1.5)/2;
			int OffSY = (LCD_getHeight() -SPECTRUM_HEIGHT*1.5)/2-1;
#endif
		const uint16_t yScr = BORDER_HEIGHT;
		uint8_t bord = SINCLAIR_FLAGS.border;
		for(int rline = 0;rline< TAPE_LINES;rline++)
			{
			//procKeyb();
			if(SINCLAIR_FLAGS.borderArray[rline].color_curr != 0xf)
			{
				bord = SINCLAIR_FLAGS.borderArray[rline].color_curr;
			}
			if(bord!=SINCLAIR_FLAGS.borderArray[rline].color_prev)
			{
				SINCLAIR_FLAGS.borderArray[rline].color_prev = bord;
				uint8_t buff[3];
				buff[0] = 0xd7*(!!(bord&2));
				buff[1] = 0xd7*(!!(bord&4));
				buff[2] = 0xd7*(!!(bord&1));
...
#ifdef ILI9341
				if((rline<yScr)||(rline>=yScr+SPECTRUM_HEIGHT))
				{
					LCD_fillRect(OffSX-BORDER_WIDTH,OffSY-BORDER_HEIGHT+rline,SPECTRUM_WIDTH+BORDER_WIDTH*2,1,color);
				}
				else
				{
					LCD_fillRect(OffSX-BORDER_WIDTH,OffSY-BORDER_HEIGHT+rline,BORDER_WIDTH,1,color);
					LCD_fillRect(OffSX+SPECTRUM_WIDTH,OffSY-BORDER_HEIGHT+rline,BORDER_WIDTH,1,color);
				}
#else
				if((rline<yScr)||(rline>=yScr+SPECTRUM_HEIGHT+SPECTRUM_HEIGHT/2))
				{
					LCD_fillRect(OffSX-BORDER_WIDTH,OffSY-BORDER_HEIGHT+rline,SPECTRUM_WIDTH+SPECTRUM_WIDTH/2+BORDER_WIDTH*2,1,color);
				}
				else
				{
					LCD_fillRect(OffSX-BORDER_WIDTH,OffSY-BORDER_HEIGHT+rline,BORDER_WIDTH,1,color);
					LCD_fillRect(OffSX+SPECTRUM_WIDTH+SPECTRUM_WIDTH/2,OffSY-BORDER_HEIGHT+rline,BORDER_WIDTH,1,color);
				}
#endif
