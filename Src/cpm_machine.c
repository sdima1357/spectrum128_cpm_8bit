...
#include "fonts.h"
#ifdef ILI9341
#define FONT_CPM  Font4x8
#else
#define FONT_CPM  FONT_CURR
#endif

#define CHAR_WIDTH_CPM (FONT_CPM.Width)
#define CHAR_HEIGHT_CPM (FONT_CPM.Height)
...
{
	LCD_Draw_Char2Font(&FONT_CURR,Character,X,Y,Colour, Size,Size,Background_Colour);
}
...
{
	LCD_Draw_Char2Font(&FONT_CURR,Character,X,Y,Colour, SizeX,SizeY,Background_Colour);
}
...
void setTermBuffer(int orient,int fontW,int fontH)
{
	...
	TBD.BW =LCD_getWidth()/(TBD.FW*CHAR_WIDTH_CPM);
	TBD.BH =LCD_getHeight()/(TBD.FH*CHAR_HEIGHT_CPM);
	TBD.currAttribute = 0;
	/*
    if(TBD.Buffer)
    {
    ...
    */
	TBD.Buffer = getRamBuffer0x4000(4);//(uint8_t *) malloc(TBD.BW*TBD.BH*2) ;
	TBD.BufferShadow = TBD.Buffer+TBD.BW*TBD.BH*2;//(uint8_t *) malloc(TBD.BW*TBD.BH*2) ;
	TBD.cursorX = 0;
	TBD.cursorY = 0;
				 ...
				 for(int x=0;x<TBD.BW;x++)
				 {
					...
					// LCD_Draw_Char2(TBD.Buffer[pos], x*TBD.FW*CHAR_WIDTH, y*TBD.FH*CHAR_HEIGHT,color_convertRGB_to16(fg), TBD.FW,TBD.FH,BLUE);
					 ...
					 if(TBD.Buffer[pos*2]!= TBD.BufferShadow[pos*2])
					 {
						 ...
						 LCD_Draw_Char2Font(&FONT_CPM,TBD.Buffer[pos*2], x*TBD.FW*CHAR_WIDTH_CPM, y*TBD.FH*CHAR_HEIGHT_CPM,FG[color>>4], TBD.FW,TBD.FH,BG[color&0xf]);
					 }
					 if(y==TBD.cursorY && x==TBD.cursorX)
					 {
						 ...
						 LCD_Draw_Char2Font(&FONT_CPM,TBD.Buffer[pos*2], x*TBD.FW*CHAR_WIDTH_CPM, y*TBD.FH*CHAR_HEIGHT_CPM,BG[color&0xf], TBD.FW,TBD.FH,FG[color>>4]);
					 }
				 }
			 }
